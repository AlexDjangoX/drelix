...
try {
  ...
  if (lowScoreProducts.length > 0) {
    lowScoreProducts.slice(0, 20).forEach((p) => {
      console.log(
        `  ${p.kod} → ${p.category} (score: ${p.score}) - ${p.nazwa.substring(0, 50)}`,
      );
    });
  ...
  console.log("\n=== SUMMARY ===");
  const otherPercent = (
    ((categoryCounts["other"] || 0) / includedRows.length) *
    100
  ).toFixed(1);
  console.log(
    `✓ ${includedRows.length - (categoryCounts["other"] || 0)} products properly categorized`,
  );
  console.log(
    `⚠ ${categoryCounts["other"] || 0} products in "other" (${otherPercent}%)`,
  );
  ...
  if (otherPercent > 5) {
    ...
    console.log(
      "   Consider adding more category rules to improve categorization.",
    );
  } else {
