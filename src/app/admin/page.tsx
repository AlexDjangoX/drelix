...
import { Upload, LogOut, Loader2, CheckCircle, AlertCircle, Pencil, Save, X, Image as ImageIcon } from "lucide-react";
      ...
      <td className="p-2 align-top text-right">
        ...
        {editing ? (
          <span className="flex gap-1 justify-end">
            <Button size="sm" variant="ghost" onClick={handleCancel} disabled={saving} aria-label="Cancel">
              <X className="w-4 h-4" />
            </Button>
...
export default function AdminPage() {
  ...
  const sections: CatalogSection[] | null = sectionsFromConvex ?? null;
  ...
  const process = async () => {
    ...
    try {
      ...
      setResult({ ok: true, message: `Catalog updated: ${rows.length} products` });
      setFile(null);
    } catch (e) {
      setResult({
        ok: false,
    ...
    <main className="container mx-auto max-w-6xl px-4 py-12">
      ...
      <section className="mb-12">
        ...
        <p className="text-muted-foreground text-sm mb-4">
          Upload a CSV (Kartoteki export, Windows-1250, semicolon-delimited). It will replace the Convex catalog. Image paths unchanged (convention: <code className="bg-muted px-1 rounded">/category/Kod.jpg</code>).
        </p>
        <div
          onDrop={onDrop}
        ...
        <div className="mt-4 flex gap-3">
          <Button onClick={process} disabled={!file || loading} className="gap-2">
            {loading ? (
              <>
        ...
        {result && (
          <div
            className={`mt-4 flex items-center gap-2 p-3 rounded-lg text-sm ${
              result.ok ? "bg-green-500/10 text-green-700 dark:text-green-400" : "bg-destructive/10 text-destructive"
            }`}
            role="alert"
          >
      ...
      <section>
        ...
        {!catalogLoading && !catalogError && sections && (
          <div className="space-y-8">
            {sections.map((section) => (
              <div key={section.slug} className="rounded-lg border border-border overflow-hidden">
                <div className="bg-muted/50 px-4 py-2 font-medium text-sm">
                  {section.slug} ({section.items.length} products)
                </div>
